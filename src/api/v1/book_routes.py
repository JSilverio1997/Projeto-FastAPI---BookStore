...
async def create_book(book_in: BookCreate) -> BookCreateOut:
...
@router.patch("/{book_id}", summary="Update some attributes a book", status_code=status.HTTP_200_OK,
              description="Updates one or more attributes of an existing book. Uses exclude_unset=True to apply partial"
                          " updates. If the book does not exist, a 404 error is returned.", response_model=BookCreateOut)
async def update_attributes_book(book_id: str, book_update: BookPatch) -> BookCreateOut:
...
async def update_replace_book(book_id: str, new_book: BookPut) -> BookCreateOut:
